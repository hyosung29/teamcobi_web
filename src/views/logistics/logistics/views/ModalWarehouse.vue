<template>
  <v-dialog id="modal-Warehouse" v-model="dialog" persistent max-width="600px">
          ...
          <v-form ref="rulesForm" lazy-validation>
            ...
            <v-row>
              <v-col class="d-flex" cols="12" sm="6">
                ...
                item-value="status"
                :items="wareStatus" label="창고 상태"></v-select>
              </v-col>
            </v-row>
          </v-form>
        </v-container>
...
export default {
  ...
  methods: {
    async fetchDetail() {
      this.loading = true
      const apiUrl = `/warehouse/${this.id}`
      try {

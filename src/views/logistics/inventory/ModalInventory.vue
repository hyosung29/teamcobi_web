...
export default {
  ...
  mounted() {
    this.fetchWarehouseName()
    this.fetchProductName()
    this.fetchSectionName()
  },
  computed: {
    _rules() {
      return {
  ...
  methods: {
    async fetchDetail() {
      this.loading = true
      const apiUrl = `/inventory/${this.id}`
      console.log(this.id)
      try {
    ...
    async fetchSectionName() {
      const apiUrl = `/section/names`
      await this.$axios
        .get(apiUrl)
        .then(({ data }) => {
          this.sectionNames = data
        })
        .catch((error) => {
          console.log(error)
        })

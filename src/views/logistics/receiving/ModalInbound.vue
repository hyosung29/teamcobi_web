<template>
    <v-dialog id="modal-inbound" v-model="dialog" persistent scrollable max-width="600px">
      <v-card v-if="loading" color="primary" dark>
        <v-card-text>
          잠시만 기다려주세요 ...
          <v-progress-linear
            indeterminate
            color="white"
            class="mb-0"
          ></v-progress-linear>
        </v-card-text>
      </v-card>
      <v-card v-else>
        <v-toolbar dense class="text-h5 mb-5" dark color="primary">
          <v-row justify="space-around">
            <v-col>입고 상세</v-col>
            <!-- <v-col>{{productId}}</v-col>
            @select="onWarehouseCapacity(warehouseCodes.volume, warehouseCodes.capacity, formData.productQuantity)"
            -->
            <v-col class="text-right">
              <v-btn x-small dark text icon @click="onClose">
                <v-icon>mdi-close-circle</v-icon>
              </v-btn>
            </v-col>
          </v-row>
        </v-toolbar>
        <v-card-text class="pt-5">
          <v-container>
            <v-form ref="rulesForm" lazy-validation>
              <v-row>
                <v-col cols="12">
                  <v-text-field
                    v-model="formData.clientName"
                    label="거래처명"
                    disabled
                    dense
                />
              </v-col>
            </v-row>
            <v-row>
              <v-col class="d-flex" cols="12" sm="6">
                <v-select
                v-model="formData.warehouseId"
                @change="onWarehouseCapacity(this.formData.warehouseId)"
                item-value="id"
                item-text="warehouseName"
                :items="warehouseCodes" label="창고명"></v-select>
              </v-col>
              <v-col class="d-flex" cols="12" sm="6">
                <v-select 
                v-model="formData.sectionId"
                item-value="sectionId"
                item-text="sectionName"
                :items="sectionCodes" label="섹션"></v-select>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12">
                <v-text-field
                  v-model="formData.createdDate"
                  :rules="_rules.required"
                  label="요청일자"
                  disabled
                  dense
                  required
                  clearable
                />
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12">
                <v-text-field
                  v-model="formData.inboundDate"
                  label="입고일자"
                  disabled
                  dense
                  clearable
                />
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12">
                <v-text-field
                  v-model="formData.status"
                  :rules="_rules.required"
                  label="상태"
                  dense
                  disabled
                  required

                />
              </v-col>
            </v-row>
            <v-row>
              <v-col class="d-flex" cols="12" sm="6">
                <v-text-field
                v-model="formData.productName"
                label="상품명"
                disabled
                dense
                clearable></v-text-field>
              </v-col>
              <v-col class="d-flex" cols="12" sm="6">
                <v-text-field
                v-model="formData.productQuantity"
                label="수량"
                disabled
                dense
                clearable></v-text-field>
              </v-col>
            </v-row>
          </v-form>
        </v-container>
      </v-card-text>
      <v-card-actions>
        <v-spacer />
        <v-btn
          color="error"
          v-if="id"
          text
          dark
          @click="showModalDeleteConfirm"
        >
          삭제
        </v-btn>
        <v-btn v-if="!id" color="blue darken-1" text dark @click="onSave">
          저장
        </v-btn>
        <v-btn v-else color="blue darken-1" text dark @click="onUpdate()">
          입고처리
        </v-btn>
        <v-btn color="gray darken-2" text @click="onClose">취소</v-btn>
      </v-card-actions>
    </v-card>
    <modal-confirm
      :id="modalConfirm.id"
      :visible.sync="modalConfirm.visible"
      :title.sync="modalConfirm.title"
      :deleteButton.sync="modalConfirm.deleteButton"
      @delete="onDelete"
    />
  </v-dialog>
</template>

<script>
import ModalConfirm from '@/components/ModalConfirm'
// const finalData = JSON.parse(JSON.stringify(this.formData));
export default {
  name: 'ModalInbound',
  components: {
    ModalConfirm,
  },
  props: {
    visible: {
      type: Boolean,
      default: false,
    },
    id: {
      type: String,
      default: null,
    },
  },
  data() {
    return {
      loading: false,
      dialog: false,
      formData: {
        clientName: null,
        clientId: null,
        productId: null,
        productQuantity: null,
        warehouseId: null,
        createdDate: null,
        inboundDate: null,
        status: null,
        sectionId: null,
        id: null,
      },
      modalConfirm: {
        id: '',
        visible: false,
        title: '',
        message: '',
        deleteButton: false,
      },
      warehouseCodes: [
      ],
      sectionCodes: [
      ],
      warehouseV: null,
      warehouseC: null,
      formQuantity: null,
      checkId: null,
    }
  },
  mounted() {
  this.fetchWarehouseCode(),
  this.fetchSectionCode()
},
  computed: {
    _rules() {
      return {
        required: [(v) => !!v || '필수값입니다. 입력해주세요!.'],
        // passwordRules: [(v) => !!v || "비밀번호를 입력해주세요!"],
      }
    },
  },
  watch: {
    visible(value) {
      this.dialog = value
      if (!value) return
      if (this.id) {
        this.$nextTick(() => {
          this.fetchDetail()
        })
      }
    },
  },
  methods: {
    async fetchDetail() {
      this.loading = true
      const apiUrl = `/inbound/${this.id}`
      try {
        await this.$axios.get(apiUrl).then(({ data }) => {
          this.formData = data
        })
      } catch (error) {
        console.log(error)
      }
      this.loading = false
    },
    async fetchWarehouseCode() {
    const apiUrl = `/warehouse/names`
    await this.$axios
    .get(apiUrl)
    .then(({ data }) => {
      this.warehouseCodes = data
  })
  .catch((error) => {
    console.log(error)
  })
  },
  async fetchSectionCode() {
    const apiUrl = `/section/names`
    await this.$axios
    .get(apiUrl)
    .then(({ data }) => {
      this.sectionCodes = data
  })
  .catch((error) => {
    console.log(error)
  })
  },
    initialize() {
      this.formData = {}
    },
    onSave() {
      if (!this.validation()) return
      this.$emit('save', this.formData)
      this.onClose()
    },
    onUpdate() {
      if (!this.validation()) return
      this.$set(this.formData, 'status', this._statusUpdate)
      this.$emit('update', this.formData)
      this.onClose()
    },
    onDelete() {
      if (!this.validation()) return
      this.$emit('delete', this.id)
      this.onClose()
    },
    onClose() {
      this.$emit('close', false)
      this.initialize()
      this.$refs.rulesForm.resetValidation()
    },
    onInsert() {
      if (!this.validation()) return
      this.$emit('insert', this.formData)
      this.onClose()
    },
    validation() {
      return this.$refs.rulesForm.validate()
    },
    onWarehouseCapacity() {
      // if((warehouseV - warehouseC) < formQuantity) {
      //   alert('선택하신 상품의 수량이 창고의 남은 용적보다 많습니다. 다른 창고를 선택해주세요.')
      alert('값 바꿨냐?ㅋ')
      var checkId = this.formData.warehouseId
      this.checkWarehouse(checkId)
      
//      }
    },
    async checkWarehouse(checkId) {
    var checkWa = checkId
    const apiUrl = `/warehouse/names/${checkWa}`
    await this.$axios
    .get(apiUrl)
    .then(({ data }) => {
      this.formQuantity = data
  })
  .catch((error) => {
    console.log(error)
  })
  },
    showModalDeleteConfirm() {
      this.modalConfirm = {
        visible: true,
        id: this.id,
        title: '삭제',
        deleteButton: true,
      }
    },
  },
}
</script>

<style></style>

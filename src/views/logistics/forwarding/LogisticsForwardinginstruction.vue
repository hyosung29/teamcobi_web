  ...
  <v-container fluid>
    ...
    <v-card class="mt-2">
      ...
      >
        ...
        <!-- v-if="[`items.status` === 'outbound']" 이거 문법이 대체 어떻게 되는거야  @click:row="test"-->
...
import ModalOutbound from './ModalOutbound.vue'
...
export default {
  ...
  methods: {
    async fetchList() {
      ...
      const apiUrl = `/outbound/hold?${queryString}`
      await this.$axios
        .get(apiUrl)
        .then((res) => {
          this.data = res.data

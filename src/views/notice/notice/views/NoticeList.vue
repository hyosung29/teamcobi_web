  ...
  <v-container fluid>
    ...
    <v-row class="pa-3" justify="start">
      ...
      >
        ...
        <v-row>
          <v-col cols="12" md="3">
            <v-text-field v-model="searchForm.title" label="제목" clearable />
          </v-col>
        </v-row>
      </search-box>
      <!-- 검색 끝 -->
    ...
    <v-card class="mt-2">
      ...
      >
        ...
        <template v-slot:[`item.id`]="{ value }">
          <v-btn dark color="blue darken-1" small @click.stop="goUpdate(value)"
            >수정</v-btn
          >
        </template>
...
export default {
  ...
  methods: {
    async fetchList() {
      ...
      const apiUrl = `/notice?${queryString}`
      await this.$axios
        .get(apiUrl)
        .then((res) => {
          // console.log(res)
    ...
    async updateNotice(value) {
      const data = value
      const apiUrl = `/notice/${this.detailId}`
      await this.$axios
        .put(apiUrl, data)
        .then(({ status }) => {
          if (status === 200) {

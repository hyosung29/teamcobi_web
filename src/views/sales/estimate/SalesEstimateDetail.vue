    ...
    <v-card>
      ...
      <v-card-actions>
        <v-btn
          ...
          color="primary"
          width="50px"
          @click="insertSalesOrderForm"
          ><v-icon left> mdi-checkbox-marked-circle </v-icon> 승인
        </v-btn>
        <v-btn class="ml-3" color="secondary" width="50px" @click="onUpdate"
...
export default {
  ...
  methods: {
    async fetchDetail() {
      this.loading = true
      const apiUrl = `/sales-estimate/${this._id}`
      await this.$axios
        .get(apiUrl)
        .then(({ data }) => {
          this.formData = data
    ...
    async insertSalesOrderForm() {
      const data = this.formData
      console.log('data', data)
      const apiUrl = `/sales-orderform/updateStatus/${this._id}`
      await this.$axios
        .put(apiUrl, data)
        .then(({ status }) => {
          if (status === 200) {

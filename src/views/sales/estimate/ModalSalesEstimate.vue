            ...
            <v-row>
              <v-col cols="12">
                <v-select
                  ...
                  :items="productCodes"
                  @change="onProductInfo(formData.productId)"
                  label="품명"
                />
              </v-col>
            </v-row>
...
export default {
  ...
  methods: {
    async fetchDetail() {
      this.loading = true
      const apiUrl = `/sales-estimate/${this.id}`
      try {
    ...
    async fetchClientCode() {
      const apiUrl = `/client/code`
      await this.$axios
        .get(apiUrl)
        .then(({ data }) => {
          console.log('data', data), (this.clientCodes = data)
        })
        .catch((error) => {
          console.log(error)
        })
    ...
    async fetchProductCode() {
      const apiUrl = `/inventory/names`
      await this.$axios
        .get(apiUrl)
        .then(({ data }) => {
          console.log('data', data), (this.productCodes = data)
        })
        .catch((error) => {
          console.log(error)
        })
    ...
    onSave() {
      ...
      this.$set(this.formData, 'inventoryId', this.productCodes.id)
    ...
    onProductInfo(id) {
      const getProductInfo = this.productCodes.find(
        (getProductInfo) => getProductInfo.productId === id
      )
      this.formData.unitPrice = getProductInfo.unitPrice
      this.formData.sectionId = getProductInfo.sectionId
      this.formData.warehouseId = getProductInfo.warehouseId
    },
    },
  }

        ...
        <v-container>
          <v-form ref="rulesForm" lazy-validation>
            ...
            <v-row>
              ...
              <v-col class="input-file" cols="12" md="6">
                <v-file-input
            ...
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field
                  v-model="formData.residentRegNum"
                  @keyup="regRRNEvent(formData.residentRegNum)"
                  :rules="_rules.residentRegNum"
                  label="주민등록번호* ('-'를 포함하여 입력해주세요)"
                  dense
                  clearable
                />
              </v-col>
              <v-col cols="12" md="6">
                <v-text-field
                  ...
                  @keyup="regPhoneEvent(formData.phone)"
                  :rules="_rules.phone"
                  label="핸드폰번호* ('-'를 포함하여 입력해주세요)"
                  dense
                  clearable
                />
              </v-col>
...
export default {
    ...
    _rules() {
      return {
        ...
        phone: [
          ...
          (v) => !(v && v.substr(0, 3) != '010') || '010 으로 시작해주세요.',
          (v) => !(v && v.substr(3, 1) != '-') || ' "-"을 포함하여 입력해주세요.',
          (v) => !(v && v.substr(8, 1) != '-') || ' "-"을 포함하여 입력해주세요.',
        ],
        residentRegNum: [
          (v) => !!v || '필수값입니다. 입력해주세요!.',
          (v) => !(v && v.length >= 15) || '주민등록번호 형식에 맞지 않습니다.',
          (v) => !(v && v.length < 14) || '주민등록번호를 입력해주세요.',
          (v) => !(v && v.substr(6, 1) != '-') || ' "-"을 포함하여 입력해주세요.',
        ],
      }
    },
  },
  ...
  methods: {
    async fetchDetail() {
      ...
      try {
        await this.$axios.get(apiUrl).then(({ data }) => {
          this.formData = data
          this.preview = `http://localhost:3001${data.photo}`
    ...
    async fetchPositionCode() {
      const apiUrl = `/position/codes`
      await this.$axios
        .get(apiUrl)
        .then(({ data }) => {

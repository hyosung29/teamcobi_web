<template>
  <v-dialog id="modal-department" v-model="dialog" persistent max-width="600px">
        ...
        <v-container>
          <v-form ref="rulesForm" lazy-validation>
            <v-row>
              <v-col cols="12">
                <v-text-field
                  v-model="formData.departmentNumber"
                  label="부서번호*"
                  dense
                  disabled
                />
...
export default {
  ...
  methods: {
    async fetchDetail() {
      this.loading = true
      const apiUrl = `/department/${this.id}`
      try {
